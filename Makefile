...
test:
	...
	docker exec constellation bash -c "cd /home/chipyard/sims/verilator && \
		...
		sed \"s/plt.savefig('noc_visualization.png')/plt.savefig('\/home\/renders\/$(name)_render.png\')/\" vis.py > vis.txt && \
		cp vis.txt vis.py && \
		...
		sed \"s/plt.savefig('\/home\/renders\/$(name)_render.png')/plt.savefig('noc_visualization.png')/\" vis.py > vis.txt && \
		cp vis.txt vis.py" 
